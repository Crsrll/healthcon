...
export default function PatientDashboard() {
  const { user } = useAuth();
  const { clinics, loading } = useClinics("approved");
  ...
  const recentlyViewed = [
    ...
    {
      id: "3",
      name: "Bukidnon Community Center",
      city: "Malaybalay",
      specialty: "General",
      image:
        ...
        <div className="relative max-w-7xl mx-auto px-6 pt-10 pb-20">
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
            ...
            <div className="flex-1">
              <p className="text-teal-300 text-[10px] font-bold uppercase tracking-[0.2em] mb-1">
                Patient Portal
              </p>
            ...
            <div className="w-full md:w-auto md:min-w-[400px]">
              <form onSubmit={handleSearch} className="flex gap-2">
                ...
                <button
                  type="submit"
                  className="bg-teal-500 hover:bg-teal-400 px-6 py-3 rounded-xl text-sm font-bold text-white transition-all shadow-lg shadow-teal-900/40"
                >
                  Search
                </button>
              </form>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          {[
            ...
            <Link
              key={item.name}
              href={item.href}
              className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm hover:shadow-md hover:border-teal-500 transition-all flex items-center gap-4 group"
            >
              <div
                className={`${item.bg} ${item.color} p-3 rounded-xl group-hover:scale-110 transition-transform`}
              >
                {item.icon}
              </div>
              ...
              <ChevronRight
                className="ml-auto text-slate-300 group-hover:text-teal-500"
                size={16}
              />
            </Link>
          ))}
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <section className="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden">
              <div className="px-6 py-5 border-b border-slate-100 flex justify-between items-center">
                ...
                <Link
                  href="/patient/appointments?timeframe=Upcoming"
              ...
              <div className="p-6 space-y-4">
                {appointments.map((apt) => (
                  <div
                    key={apt.id}
                    className="flex items-center gap-4 p-4 rounded-2xl border border-slate-50 bg-slate-50/30 hover:bg-white hover:border-teal-200 transition-all group"
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        ...
                        <span
                          className={`text-[9px] px-2 py-0.5 rounded-full font-black uppercase ${apt.status === "Confirmed" ? "bg-green-100 text-green-700" : "bg-amber-100 text-amber-700"}`}
                        >
                          {apt.status}
                        </span>
                      </div>
                    ...
                    <button
                      ...
                      className="text-xs font-bold text-slate-400 group-hover:text-teal-600 transition-colors"
                    >
                      Details →
                    </button>
                  </div>
                ...
                {prescriptions.map((med) => (
                  ...
                  >
                    ...
                    <div>
                      ...
                      <span className="inline-block mt-1 text-[9px] font-bold text-teal-600 bg-purple-50 px-2 py-0.5 rounded-md">
                        {med.remaining}
                      </span>
                    </div>
                  </div>
                ...
                {loading ? (
                  ...
                  clinics.slice(0, 2).map((clinic) => (
                    <Link
                      ...
                      key={clinic.id}
                      className="bg-white rounded-3xl overflow-hidden border border-slate-200 hover:shadow-lg transition-all group"
                    >
                      <div className="relative h-40 overflow-hidden">
                        ...
                        <span className="absolute bottom-3 left-4 text-[10px] font-black text-white bg-teal-500 px-3 py-1 rounded-full uppercase tracking-widest">
                          {Array.isArray(clinic.specialty)
                            ? clinic.specialty[0]
                            : clinic.specialty}
                        </span>
                      </div>
          ...
          <aside className="space-y-6">
            ...
            <section className="bg-white rounded-3xl border border-slate-200 shadow-sm p-6">
              ...
              <p className="text-[11px] text-slate-500 leading-relaxed mb-5">
                Complete your medical history to speed up your next clinic
                check-in.
              </p>
              <Link
                href="/patient/profile"
                className="block w-full py-2 text-center bg-slate-50 hover:bg-teal-50 text-teal-600 text-[10px] font-black uppercase tracking-widest rounded-xl border border-teal-100 transition-all"
              >
                Finish Setup
              </Link>
            </section>
            ...
            <div className="bg-gradient-to-br from-[#1a365d] to-[#2d4a77] rounded-2xl p-6 text-white shadow-xl relative overflow-hidden">
              ...
              <p className="text-teal-300 text-[10px] font-black uppercase tracking-widest mb-3">
                Daily Health Tip
              </p>
